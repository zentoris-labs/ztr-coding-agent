...
- A host bootstrap script (`infra/bootstrap.sh`) that installs Docker + sysbox-runc + Tailscale on a Linux host
- A single-agent compose template (`docker-compose.yml`) — sysbox-runc runtime baked in
...
| **Personal identifying info** | Developer real names in code (placeholder names like alice/bob/carol are fine for examples), email addresses other than generic role addresses, personal SSH **public** keys | Privacy and onboarding-leak concerns |
...
├── docker-compose.yml               # single-agent template; runtime: sysbox-runc baked in
├── .env.example                     # full env contract — every variable documented
├── infra/bootstrap.sh               # idempotent host setup (Docker CE + sysbox-runc + Tailscale)
├── infra/README.md                  # 4-step manual host setup walkthrough
...
**Deployment model** (manual, 4 steps, in [README.md](README.md)):
1. Operator creates a Linux host (Hetzner / office / etc.)
2. Operator runs `infra/bootstrap.sh` on the host (installs Docker + sysbox-runc + Tailscale)
3. Operator writes a per-host `docker-compose.yml` ON the host (or in a private ops repo) with per-agent `.env` files (each carrying a `TS_AUTHKEY`)
4. `docker compose up -d`; each agent joins the tailnet; access via `ssh agent@agent-<owner>-<NN>` (resolves via Tailscale MagicDNS)
...
- **Workspace:** `/workspace/` (per-agent named volume)
...
3. **Don't enable `--privileged` in compose.** Use `runtime: sysbox-runc` (already in `docker-compose.yml`) for nested Docker. `--privileged` trades isolation for convenience — wrong trade-off when prompt injection is part of the threat model.
...
5. **Don't reintroduce per-agent permanent branches.** Each agent has its own `/workspace/` volume; per-task feature branches are the model. The old `agent-<owner>-<NN>`-permanent-branch sketch was scrapped.
6. **Don't hardcode specific repo names, hostnames, or developer identifiers in examples.** Use placeholder names (alice / bob / carol).

## Build / test loop

- **Local rebuild:** `docker compose build`
- **Quick smoke test:** `docker compose up -d` then `docker logs agent-al-01` — expect the entrypoint stages + `tailnet hostname: agent-al-01`.
- **Full validation:** requires a Linux host with sysbox-runc installed. Docker Desktop WSL2 cannot run nested Docker without `--privileged`, so in-container Testcontainers / docker-in-docker only works on a real Linux host.
...
1. **`docker compose build`** locally and read entrypoint logs from `docker logs agent-al-01` to confirm nothing regressed.
...
- **Sysbox (nested Docker isolation):** [github.com/nestybox/sysbox](https://github.com/nestybox/sysbox)
